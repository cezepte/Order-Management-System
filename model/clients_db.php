...
class Clients{
    ...
    public function insert_new_client($firstName, $lastName, $company, $tin, $tel_number, $email){
        global $db;
        $count = 0;
        $query = "INSERT INTO clients(firstName,lastName,company,tin,tel_number,email) VALUES (:firstName, :lastName, :company, :tin, :tel_number, :email)";
        $statement = $db->prepare($query);
        $statement->bindValue(':firstName', $firstName, PDO::PARAM_STR);
        $statement->bindValue(':lastName', $lastName, PDO::PARAM_STR);
        $statement->bindValue(':company', $company, PDO::PARAM_STR);
        $statement->bindValue(':tin', $tin, PDO::PARAM_INT);
